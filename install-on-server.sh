#!/bin/bash
# Этот скрипт создаст setup-server-remote.sh на сервере

cat << 'SCRIPT' | ssh root@45.137.151.98 "cat > ~/setup-server-remote.sh"
#!/bin/bash
...
echo "Скрипт загружен на сервер. Теперь выполните на сервере:"
echo "  chmod +x ~/setup-server-remote.sh"
echo "  ./setup-server-remote.sh"
